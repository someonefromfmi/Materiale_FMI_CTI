...
int main(int argc, char *argv[])
{
    ...
    work = gsl_fft_real_workspace_alloc(local_N);
    real = gsl_fft_real_wavetable_alloc(local_N);
    gsl_fft_real_transform(local_data, 1, local_N, real, work);
    gsl_fft_real_wavetable_free(real);

    // filtrare
    for (i = FILTER; i < local_N; i++) {
        local_data[i] = 0;
    }

    // transformata inversa
    hc = gsl_fft_halfcomplex_wavetable_alloc(local_N);
