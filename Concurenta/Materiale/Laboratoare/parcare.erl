...
main(_) -> 
  ...
  io:format("Legenda:~n"),
  io:format("Student = Car~n"),
  io:format("Parcare = Terminal~n"),
  io:format("open = idle~n"),
  io:format("full = in_use~n"),
  io:format("waiting = waiting~n"),
  io:format("parking = searching~n"),
...
% Intr-o biblioteca, mai multi stud solicita acces la un singur terminal
% de cautare electronica
% Acest terminal are doua stari,
% - idle: in care asteapta cereri de acces din partea studentilor
% - in use: in care permite unui singur stud sa caute info timp de 3 secunde
% Un stud poate fi in starea
% waiting: daca terminalul este ocupat
% searching: cand are acces la terminalul