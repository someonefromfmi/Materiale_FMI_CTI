...
data Tree a = Level [a] (Maybe (Tree a))

exLevelTree :: Tree Integer
exLevelTree = Level [2,4] (Just (Level [3] (Just (Level [6,15] Nothing))))

-- 1
instance Show a => Show (Tree a) where
    show (Level l Nothing) = show l
    show (Level l (Just t)) = show l ++ " -> " ++ show t

-- 2
treeToListsOfLists :: Tree a -> [[a]]
treeToListsOfLists (Level l Nothing) = [l]
treeToListsOfLists (Level l (Just t)) = l : treeToListsOfLists t

-- 3
instance Semigroup (Tree a) where
    (Level l Nothing) <> (Level l1 Nothing) = Level (l <> l1) Nothing
    (Level l (Just t)) <> (Level l1 Nothing) = Level (l <> l1) (Just t)
    (Level l Nothing) <> (Level l1 (Just t)) = Level (l <> l1) (Just t)
    (Level l (Just t)) <> (Level l1 (Just t1)) = Level (l <> l1) (Just (t <> t1))

instance Monoid (Tree a) where
    mempty = Level [] Nothing

instance Foldable Tree where
    foldr _ acc (Level [] Nothing) = acc
    foldr f acc (Level [] (Just t)) = foldr f acc t
    foldr f acc (Level l Nothing) = foldr f acc l
    foldr f acc (Level l (Just t)) = foldr f (foldr f acc l) t

--4
removeLevelWithNElemsComp :: Tree a -> Int -> [[a]]
removeLevelWithNElemsComp t n' = f (treeToListsOfLists t) n'
    where
        f ll n = [l | l <- ll, length l /= n]

removeLevelWithNElemsHOF :: Tree a -> Int -> [[a]]
removeLevelWithNElemsHOF t n' = f (treeToListsOfLists t) n'
    where
        f ll n = filter (\l -> length l /= n) ll

-- 5
treeMap :: (a -> b) -> Tree a -> Tree b
treeMap f (Level l Nothing) = Level (map f l) Nothing
treeMap f (Level l (Just t)) = Level (map f l) (Just (treeMap f t))

newtype Writer w ctx a = Writer { runWriter :: (w, Maybe a, ctx) } deriving Show

instance Functor (Writer w ctx) where
    fmap f (Writer (w, ma, ctx)) = Writer (w, fmap f ma, ctx)

instance (Monoid w, Monoid ctx) => Applicative (Writer w ctx) where
    pure x = Writer (mempty, pure x, mempty)
    (Writer (w, f, ctx)) <*> (Writer (w', x, ctx')) = Writer (w <> w', f <*> x, ctx <> ctx')

instance (Monoid w, Monoid ctx) => Monad (Writer w ctx) where
    return = pure
    (Writer (w, ma, ctx)) >>= f = 
        case ma of
            Nothing -> Writer (w, Nothing, ctx)
            Just a -> let Writer (w', mb, ctx') = f a
                        in Writer (w <> w', mb, ctx <> ctx')

-- III
parseList :: Parser [Int]
parseList = do
    char '['
    nums <- sepBy' integer (char ',' >> return ())
    char ']'
    return nums
...
parseTree = do
    ...
    rest <- (do
        arrow
        subTree <- parseTree
        return (Just subTree)) `alt` return Nothing
    return (Level firstList rest)

example :: String
example = "[1,2] -> [3] -> [4,5]"

treeParsed :: Tree Int
treeParsed = parse parseTree example
