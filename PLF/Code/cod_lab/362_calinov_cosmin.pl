...
wroteBy(Name, Res) :-
    findall(Book, (books(_, Book, AuthId, _, _), authors(AuthId, Name, _)), Res).
...
editionAfterYear(Year, Res) :-
    findall((BookName, AuthName), 
    (books(BookId, BookName, AuthId, _, _), authors(AuthId, AuthName, _), editions(_, BookId, _, EYear), EYear >= Year), 
    Res).
...
countByCountries(Result) :-
    findall(Country, (books(_, _, _, PubId, _), publishers(PubId, _, Country)), AllCountries),
    sort(AllCountries, UniqueCountries),
    findall((Country, Count), 
            (member(Country, UniqueCountries), 
             findall(C, (books(_, _, _, PubId, _), publishers(PubId, _, C), C = Country), CountryList),
...
splitListEvenOdd([], [], []).
splitListEvenOdd([H|T], [H|TR], OddList) :-
    H mod 2 =:= 0,
    splitListEvenOdd(T, TR, OddList).
splitListEvenOdd([H|T], EvenList, [H|TR]) :-
    H mod 2 =:= 1,
    splitListEvenOdd(T, EvenList, TR).
...
% false.
findSublist(_, Num, Num, []).
findSublist([H|T], Num, Acc, [H|TR]) :-
    NewAcc is H + Acc,
    NewAcc =< Num,
    findSublist(T, Num, NewAcc, TR).
findSublist([_|T], Num, Res) :-
    findSublist(T, Num, 0, Res).
findSublist(L, Num, Res) :- 
    findSublist(L, Num, 0, Res).
% [3 puncte]
...
solution(L, S, Sol) :-
    findSublist(L, S, Sol),
    all_even(Sol),
    findSublist(L, S, OddSol),
    all_odd(OddSol),
    length(OddSol, OddLen),
    length(Sol, EvenLen),
    OddLen =< EvenLen.
...
solution(L, S, Sol) :-
    findSublist(L, S, Sol),
    all_odd(Sol),
    findSublist(L, S, EvenSol),
    all_even(EvenSol),
    length(EvenSol, EvenLen),
    length(Sol, OddLen),
    EvenLen < OddLen.
...
% false 
literalForResolution([H1|_], C2, H1) :-
    member(non(H1), C2), !.
literalForResolution([non(H1)|_], C2, H1) :-
    member(H1, C2), !.
literalForResolution([_|T1], C2, Res) :-
    literalForResolution(T1, C2, Res).
...
% false
rezolvent(C1, C2, Res) :-
    literalForResolution(C1, C2, Lit),
    select(Lit, C1, Res1),
    select(non(Lit), C2, Res2),
    union(Res1, Res2, Res).
rezolvent(C1, C2, Res) :-
    literalForResolution(C1, C2, Lit),
    select(non(Lit), C1, Res1),
    select(Lit, C2, Res2),
    union(Res1, Res2, Res).
